...
public:
    ...
    void agregar();
    float getCosto();
	void confirmacion(string f, string t, float c, string d);
...
void Servicio::agregar(){
		cout << "Ingresa lo que se te pide sobre el servicio: \n Fecha(dd/mm/aaaa): ";
		cin >> fecha;
		cout <<"Tipo de servicio: "; cin >>tipo;
		cout << "Decripcion: "; cin >> descripcion;
		cout << "Precio: "; cin >> costo;
		confirmacion(fecha, tipo, costo, descripcion);
}
void Servicio::confirmacion(string f, string t, float c, string d){
	char sn='s';
	cout<<"\nLos datos del Servicio son: ";
	cout<<"\nFecha: "<<f;
	cout<<"\nTipo: "<<t;
	cout<<"\nDescripcion: "<<d;
	cout<<"\nCosto: "<<c;
...
void Servicio::editarInformacion(){
	string mod;
	string opc;
	cout<<"\nQue dato quieres modificar? (fecha/tipo/descripcion/costo) ";
	cin>>mod;
	if (mod=="fecha"){
		...
		cout<<"Tipo: (mantenimiento/reparacion) ";
		cin>>tipo;
	}
	else if (mod=="descripcion"){
		cout<<"Descripcion: ";
		cin>>descripcion;
	}
	else if (mod=="costo"){
		...
		cout<<"Ese no es un valor, intente de nuevo";
	}
	confirmacion(fecha, tipo, costo, descripcion);
}
...
void Vehiculo::mostrar(){
    if(modelo!=""){
        ...
        cout << setw(10) << "Kilometros: " << setw(12) << kilometros << "\nServicios: \n";
        servicio.muestra();
    }
}
void Vehiculo::agregar(){
    ...
    cout << "Marca: ";
    cin >> marca;
    cout << "Tipo de auto (estandar/automatico): ";
    cin >> tipo;
    cout << "Color: ";
    cin >> color;
    cout << "Placas: ";
...
int Persona::estaCliente(int id){
...
void Persona::borrar(int id){
    ...
    if(p != agenda.end())
        ...
        cout << id << " no esta en el directorio.\n";
    ...
    switch(des){
        case 1:
			...
			while (opc == "s" || opc== "S"){
				...
				cin>>opc;
				if(opc=="s" || opc=="N"){
					ganancia=ganancia+c.getV()->getServ().precio();
					c.getV()->agregarServ();
				}
				c.muestra();
			}
        ...
        case 2:
            ...
            if(!co.estaVacia()){
                if(c.getNom()!=co.datoFte().getNom()){
                    c.muestra();
                    cout << "\nAgrega el servicio que se le dara esta vez:\n";
                    c.getV()->agregarServ();
                    x.borrar(id);
                    x.ingresar(id, c);
                    co.mete(x.encontrar(id));
        ...
        case 3:
            if(!co.estaVacia()){
                ...
                cout << "\nError, no tenias a nadie a quien atender\n";
            break;
        case 4:
            co.mostrar();
            break;
